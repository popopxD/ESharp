            ...
            for (uint num = 0; num < 10; num++)
            {
                try
                {
                    if (Heroes[num] == null || !Heroes[num].IsValid)
                        Heroes[num] = ObjectManager.GetPlayerByID(num).Hero;
                }
                catch
                {
                    continue;
                ...
                int i2 = 0;
                int inventoryLength = Heroes[num].Inventory.Items.Count();
                foreach (var item in Heroes[num].Inventory.Items.Union(Heroes[num].Inventory.StashItems) )
                {
                    ++i2;
                    if (i2 == inventoryLength) ++i2;
                    try
                    {
                        string texturename;
                        if (item == null)
